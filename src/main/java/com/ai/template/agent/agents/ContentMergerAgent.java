...
public class ContentMergerAgent implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        log.info("ContentMergerAgent 开始执�? 正文长度={}, 图片数量={}", content.length(), images.size());
        ...
        for (ArticleState.ImageResult image : images) {
            ...
            if (placeholder != null && !placeholder.isEmpty()) {
                ...
                if (fullContent.contains(placeholder)) {
                    ...
                    log.info("成功替换占位�? {} -> {}", placeholder, imageMarkdown.substring(0, Math.min(50, imageMarkdown.length())));
                } else {
                    log.warn("正文中未找到占位�? {}", placeholder);
                }
            } else {
                log.warn("图片 position={} �?placeholderId 为空", image.getPosition());
            }
        }
