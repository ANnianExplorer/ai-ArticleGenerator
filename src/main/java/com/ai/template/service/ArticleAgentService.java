...
public class ArticleAgentService {
    ...
    public void executePhase1_GenerateTitles(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            log.info("阶段1：标题方案生成完�? taskId={}, optionsCount={}", 
                state.getTaskId(), state.getTitleOptions().size());
        } catch (Exception e) {
            log.error("阶段1：标题方案生成失�? taskId={}", state.getTaskId(), e);
    ...
    public void executePhase2_GenerateOutline(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            log.info("阶段2：大纲生成完�? taskId={}", state.getTaskId());
        } catch (Exception e) {
            log.error("阶段2：大纲生成失�? taskId={}", state.getTaskId(), e);
    ...
    public void executePhase3_GenerateContent(ArticleState state, Consumer<String> streamHandler) {
        try {
            ...
            log.info("阶段3：开始图文合�? taskId={}", state.getTaskId());
            proxy.mergeImagesIntoContent(state);
            streamHandler.accept(SseMessageTypeEnum.MERGE_COMPLETE.getValue());

            log.info("阶段3：正文生成完�? taskId={}", state.getTaskId());
        } catch (Exception e) {
            log.error("阶段3：正文生成失�? taskId={}", state.getTaskId(), e);
    ...
    public void agent1GenerateTitleOptions(ArticleState state) {
        ...
        log.info("智能�?：标题方案生成成�? optionsCount={}", titleOptions.size());
    }
    ...
    public void agent2GenerateOutline(ArticleState state, Consumer<String> streamHandler) {
        ...
        log.info("智能�?：大纲生成成�? sections={}", outlineResult.getSections().size());
    }
    ...
    public void agent3GenerateContent(ArticleState state, Consumer<String> streamHandler) {
        ...
        log.info("智能�?：正文生成成�? length={}", content.length());
    }

    
    @AgentExecution(value = "agent4_analyze_image_requirements", description = "分析配图需�?)
    public void agent4AnalyzeImageRequirements(ArticleState state) {
        ...
        log.info("智能�?：配图需求分析成�? count={}, validated={}, 已在正文中插入占位符", 
    ...
    public void agent5GenerateImages(ArticleState state, Consumer<String> streamHandler) {
        ...
        for (ArticleState.ImageRequirement requirement : state.getImageRequirements()) {
            ...
            log.info("智能�?：开始获取配�? position={}, imageSource={}, keywords={}", 
            ...
            log.info("智能�?：配图获取并上传成功, position={}, method={}, cosUrl={}", 
                    requirement.getPosition(), method.getValue(), cosUrl);
        }
        
        state.setImages(imageResults);
        log.info("智能�?：所有配图生成并上传完成, count={}", imageResults.size());
    }
    ...
    private String getAllMethodsDescription() {
        return """
               ...
               - ICONIFY: 适合图标、符号、小型装饰性图标（如：箭头、勾选、星星、心形等�?               - EMOJI_PACK: 适合表情包、搞笑图片、轻松幽默的配图
    ...
    private String getMethodDetailedGuide(String method) {
        return switch (method) {
            case "PEXELS" -> """
                    - PEXELS: 提供英文搜索关键�?keywords)，要准确、具体。prompt 留空�?"";
            case "NANO_BANANA" -> """
                    - NANO_BANANA: 提供详细的英文生图提示词(prompt)，描述场景、风格、细节。keywords 留空�?"";
            case "MERMAID" -> """
                    - MERMAID: �?prompt 字段生成完整�?Mermaid 代码（如流程图、架构图）。keywords 留空�?"";
            case "ICONIFY" -> """
                    - ICONIFY: 提供英文图标关键�?keywords)，如：check、arrow、star、heart。prompt 留空�?"";
            case "EMOJI_PACK" -> """
                    - EMOJI_PACK: 提供中文或英文关键词(keywords)描述表情内容。prompt 留空。系统会自动添加"表情�?搜索�?"";
            case "SVG_DIAGRAM" -> """
                    - SVG_DIAGRAM: �?prompt 字段描述示意图需求（中文），说明要表达的概念和关系。keywords 留空�?                      示例：绘制思维导图样式的图，中心是"自律"，周�?个分支：习惯、环境、反馈、系�?"";
            default -> null;
        };
    }
    ...
    private List<ArticleState.ImageRequirement> validateAndFilterImageRequirements(
        ...
        for (ArticleState.ImageRequirement req : requirements) {
            ...
            if (enabledMethods.contains(imageSource)) {
                ...
                log.warn("配图需求不符合限制被过�? position={}, imageSource={}, enabledMethods={}", 
    ...
    private ArticleAgentService getProxy() {
        try {
            ...
            log.warn("获取 AOP 代理对象失败，使用原始对�? {}", e.getMessage());
            return this;
        }
    }
